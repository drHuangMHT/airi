...
export function useExpressionController(options: ExpressionControllerOptions) {
  ...
  function applyExpressions(coreModel: PixiLive2DInternalModel['coreModel']) {
    ...
    for (const entry of store.expressions.values()) {
      ...
      coreModel.setParameterValueById(CubismFramework.getIdManager().getId(entry.parameterId), blendedValue)
    ...
    for (const paramId of activeLastFrame) {
      if (!activeThisFrame.has(paramId)) {
        ...
        if (entry)
          coreModel.setParameterValueById(CubismFramework.getIdManager().getId(paramId), entry.modelDefault)
      }
    }
  ...
  function computeTargetValue(entry: ExpressionEntry, coreModel: PixiLive2DInternalModel['coreModel']): number {
    switch (entry.blend) {
      ...
      case 'Multiply': {
        const currentFrameValue = coreModel.getParameterValueById(CubismFramework.getIdManager().getId(entry.parameterId)) as number
  ...
  function getModelParameterDefault(parameterId: string): number {
    ...
    try {
      ...
      // the default.
      return (im.coreModel.getParameterValueById(CubismFramework.getIdManager().getId(parameterId)) as number) ?? 0
    }
    catch {
      return 0
    }
