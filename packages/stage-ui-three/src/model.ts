...
const METADATA_STORAGE_KEY = 'plugin.airi_plugin_stage_three.model.metadata.'
const FILE_STORAGE_KEY = 'plugin.airi_plugin_stage_three.model.file.'
const PREVIEW_STORAGE_KEY = 'plugin.airi_plugin_stage_three.model.preview.'
const SELECTED_MODEL_STORAGE_KEY = 'plugin.airi_plugin_stage_three.model.selected_model.'
...
export const useModelsStore = defineStore('airi_plugin_stage_three.models', () => {
  ...
  async function addModel(metadata: ModelMetadata, file: File, preview: Blob | null = null): Promise<string | null> {
    ...
    if (maybeMetadata)
      return null
    preview = preview ?? await loadVrmModelPreview(file)
    if (!preview)
      return null
    try {
      await Promise.all([
